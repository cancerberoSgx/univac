...
test('should output to stdout if no --output', async t => {
  const result = execSync('node bin/render-dot.js --input test/assets/C.blockItem.dot', { stdio: 'pipe' }).toString()
  const expected = [`<polygon`, `<svg`]
  expected.forEach(e => t.true(result.includes(e)))
})

test('should accept globs and output folder', async t => {
  const result = execSync('rm -rf tmp; mkdir -p tmp; node bin/render-dot.js --input "test/**/*.dot" --output tmp', { stdio: 'pipe' }).toString()
...
test('should get code from --input', async t => {
  const result = execSync('node bin/render-dot.js --input "digraph { a -> b; }"', { stdio: 'pipe' }).toString()
  const expected = [`<polygon`, `<svg`]
  expected.forEach(e => t.true(result.includes(e)))
})
...
test('--format xdot should work', async t => {
  const result = execSync('node bin/render-dot.js --input "digraph { a -> b; }" --format xdot', { stdio: 'pipe' }).toString()
  const expected = [`graph [`, `a -> b	 [`]
  expected.forEach(e => t.true(result.includes(e)))
})

test('--format plain should work', async t => {
  const result = execSync('node bin/render-dot.js --input "digraph { a -> b; }" --format plain', { stdio: 'pipe' }).toString()
  const expected = [`
graph 1 0.75 1.5
